...
class VerticalLayout(val gap: Int = 0) : AbstractLayout() {
    ...
    override fun preferredLayoutSize(target: Container): Dimension {
        ...
        size.height += max(visibleCount - 1, 0) * gap
    ...
    override fun layoutContainer(target: Container) {
        ...
        for (index in 0..<target.componentCount) {
            ...
            component.setBounds(bounds.x, bounds.y + offset, bounds.width, preferredSize.height)

            offset += preferredSize.height + gap
        }
    }
}
